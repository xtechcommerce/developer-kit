...
run_search_query = function(){
	...
	$.post(baseurl + "index.php/search/find/", { name: $('#search-engine').val(), limit: 10 },
		function(data){
			...
			$.each(data, function(index, product){
				...
				if(index == 'count'){
					...
					if($('.search-result-'+index).length == 0){
						...
						$('#search-results-in').append('<a href="' + baseurl + product.slug + '" id="search-result-' + num + '" class="search-result-' + index +'">' +
															'<div class="line">' +
